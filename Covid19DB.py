...
def process_data(data,cntry='Bangladesh',window=3):
    conf_ts = data
...
def get_cntry_total(df,cntry='Bangladesh'):
...
def fig_world_trend(cntry='Bangladesh',window=3):
...
def graph1():
    return dcc.Graph(id='graph1',figure=fig_world_trend('Bangladesh'))
...
def generate_cards(cntry='Bangladesh'):
    conf_cntry_total = get_cntry_total(covid_conf_ts,cntry)
    dead_cntry_total = get_cntry_total(covid_dead_ts,cntry)
    recv_cntry_total = get_cntry_total(covid_recv_ts,cntry)
    cards = html.Div(
